...
#             checkpoint = deepspeed.checkpointing.checkpoint
    ...
    def forward(self, hidden_states, ltor_mask, pivot_idx=None, is_sparse=0, mem=None):
        ...
        if mem is None:
            ...
            cat = torch.cat((mem, hidden_states), 1, device=xm.xla_device())
...
class GPT2ParallelMLP(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        intermediate_parallel = intermediate_parallel.to(device)
    ...
    def forward(self, hidden_states, position_ids, attention_mask, txt_indices_bool, img_indices_bool, is_sparse=0, *mems):
        ...
        if isinstance(attention_mask, int) or attention_mask.numel() == 1:
            ...
            def build_mask_matrix(query_length, key_length, sep):
                ...
                m[0, :, -query_length:] = torch.tril(m[0, :, -query_length:], device=xm.xla_device())
        ...
        if is_sparse == 1 and (self.rmask is None):
            ...
            tmp = torch.tril(1 - torch.block_diag(*tmp), device=xm.xla_device())
        ...
        if self.checkpoint_activations:
            ...
            while l < num_layers:
                if is_sparse > 0:
                    ...
                    if is_sparse == 1: # sparse training
                        ...
                        pw_idx = torch.cat((pivot_idx, window_idx), dim=-1, device=xm.xla_device())
            ...
            for i, layer in enumerate(self.layers):
                if is_sparse == 0:
                    ...
                    pw_idx = torch.cat((pivot_idx, window_idx), dim=-1, device=xm.xla_device())
                    args = [hidden_states, attention_mask_saved, pw_idx, torch.tensor(is_sparse, device=xm.xla_device())]
