...
#             device = torch.device('cuda', device)
        ...
        def cb():
            ...
            if idx is None:
                idx = device
            default_generator = torch.xla.default_generators[idx]
    ...
    if torch_xla.distributed.get_rank() == 0:
        print('> initializing model parallel cuda seeds on global rank {}, '
              ...
              'model parallel seed: {} and data parallel seed: {}'.format(
                  torch_xla.distributed.get_rank(), get_model_parallel_rank(),
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, *args):
        ...
        if cuda_device is None:
            ...
            transport_stream = torch.Stream(device=device)
        ...
        inputs_cuda = [item.to(device) for item in args]
...
#         ctx.fwd_cuda_rng_state = torch.get_rng_state()
        ...
        if PARTITION_ACTIVATIONS:
            ...
            for arg, inp in zip(args,inputs):                
                size= torch.tensor(arg.size(), device=xm.xla_device())
                arg.data = inp.data
                new_args.append(arg)
...
#         bwd_cuda_rng_state = torch.cuda.get_rng_state()
        ...
        _set_cuda_rng_state(ctx.fwd_cpu_rng_state)
        get_cuda_rng_tracker().set_states(ctx.fwd_cuda_rng_state_tracker)
        
        if PARTITION_ACTIVATIONS:
            current_stream=torch.current_stream()
        ...
        _set_cuda_rng_state(bwd_cpu_rng_state)
