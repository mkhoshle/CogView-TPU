...
def get_masks_and_position_ids(data,
    ...
    if args is not None and args.finetune and args.max_position_embeddings < args.max_position_embeddings_finetune:
        ...
        tmp = torch.nonzero(data == start_token, as_tuple=False, device = xm.xla_device())
...
def forward_step(data_iterator, model, args, timers, mems):
    ...
    torch_xla.distributed.all_reduce(img_loss.data)
    torch_xla.distributed.all_reduce(txt_loss.data)
...
def backward_step(optimizer, model, lm_loss, args, timers):
    ...
    # Reduce losses for logging
    torch_xla.distributed.all_reduce(reduced_losses.data)
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            if isinstance(model, DDP):
                torch_xla.distributed.all_reduce(lm_loss.data)
...
def get_train_val_test_data(args):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        token_counts = torch.LongTensor(
            [after, int(args.do_train), int(args.do_valid), int(args.do_test)],device = xm.xla_device())
    else:
        token_counts = torch.LongTensor([0, 0, 0, 0],device = xm.xla_device())
    # Broadcast num tokens.
    torch_xla.distributed.broadcast(token_counts,
