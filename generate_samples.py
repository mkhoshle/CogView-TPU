            ...
            if args.generation_task == 'post-selection':
                ...
                for part in parts[1:]:
                    try:
                        ...
                        print(e)
                        continue
                seq = torch.stack(seqs, device=xm.xla_device())
            else:
                try:
                    seq = _parse_and_to_tensor(raw_text, img_size=img_size, query_template=query_template)
                except (ValueError, FileNotFoundError) as e:
...
def generate_images_once(model, args, raw_text, seq=None, num=8, query_template='{}', output_path='./samples'):
    ...
    with torch.no_grad():
        ...
        for seq in output_tokens_list:
            ...
            for i in range(len(decoded_imgs)):
                if decoded_imgs[i].shape[-1] == 128:
                    decoded_imgs[i] = torch.nn.functional.interpolate(decoded_imgs[i], size=(256, 256), device=xm.xla_device())
            if args.debug:
        ...
        if args.debug:
            ...
            imgs = torch.cat(imgs, dim=0, device=xm.xla_device())
            print(txts)
            ...
            save_image(torch.cat(imgs, dim=0, device=xm.xla_device()), os.path.join(output_path,f'concat.jpg'), normalize=True)
...
def post_selection(model, args, raw_text, seq, output_path):
    ...
    with torch.no_grad():
        ...
        scores = torch.cat(scores, dim=0, device=xm.xla_device())
